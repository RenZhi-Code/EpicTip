...
-- Check if this module should be enabled (Retail only - requires modern stat system)
local isRetailOnly = WOW_PROJECT_ID == WOW_PROJECT_MAINLINE

if not isRetailOnly then
    -- Classic/Era doesn't have Versatility, Mastery, or tertiary stats
    -- Disable the entire module
    StatValues.Initialize = function()
        if EpicTipDB and EpicTipDB.debugMode then
            print("EpicTip: StatValues module disabled (Classic/Era detected - retail-only feature)")
        end
    end

    -- Create empty stub to prevent errors
    EpicTipStatValuesMixin = {
        ProcessStatTooltip = function() end
    }
...
-- Conversion factors from SimulationCraft official data + Midnight extrapolation
-- Source: https://github.com/simulationcraft/simc/blob/thewarwithin/engine/dbc/generated/sc_scale_data.inc
-- Build: 11.2.7.64772 (Last updated: 2024-12-30)
-- IMPORTANT:
-- - Levels 1-80: Official SimC data (7% error fixed for level 80)
-- - Levels 81-90: EXTRAPOLATED estimates based on 71-80 growth patterns (13.1% per level)
--   These are TEMPORARY until Blizzard finalizes Midnight beta stat formulas
--   Growth model: exponential progression matching War Within expansion curve
local CONVERSION_FACTORS = {
    ...
    haste = {
        ...
        379.3451222, 423.7760322, 473.4109257, 660, -- 80 (SimC official: was 706, fixed)
        746, 844, 955, 1080, 1221, 1381, 1562, 1767, 1999, 2261 -- 81-90 (EXTRAPOLATED - pending Midnight beta)
    },
    ...
    crit = {
        ...
        402.3357357, 449.4594281, 502.102497, 700, -- 80 (SimC official: was 749, fixed)
        792, 896, 1013, 1146, 1296, 1466, 1658, 1875, 2120, 2398 -- 81-90 (EXTRAPOLATED - pending Midnight beta)
    },
    ...
    versatility = {
        ...
        448.3169626, 500.8262199, 559.4856395, 780, -- 80 (SimC official: was 835, fixed)
        902, 1043, 1206, 1394, 1612, 1864, 2155, 2492, 2881, 3331 -- 81-90 (EXTRAPOLATED - pending Midnight beta)
    },
    ...
    leech = {
        ...
        586.2578415, 654.9234651, 731.6315702, 1020, -- 80 (Maxroll official: was 1091, fixed)
        1179, 1363, 1576, 1822, 2107, 2436, 2816, 3255, 3763, 4351 -- 81-90 (EXTRAPOLATED - pending Midnight beta)
    },
    ...
    avoidance = {
        ...
        312.6708488, 349.2925147, 390.2035041, 544, -- 80 (SimC/Maxroll official: was 582, fixed)
        629, 727, 841, 972, 1124, 1300, 1503, 1738, 2009, 2323 -- 81-90 (EXTRAPOLATED - pending Midnight beta)
    },
    ...
    speed = {
        ...
        97.70964024, 109.1539108, 121.938595, 170, -- 80 (SimC/Maxroll official: was 182, fixed)
        197, 227, 263, 304, 352, 407, 470, 544, 629, 727 -- 81-90 (EXTRAPOLATED - pending Midnight beta)
    }
}
...
function EpicTipStatValuesMixin:GetConversionFactor(statType)
    ...
    level = math.max(math.min(level, 90), 1) -- Clamp between 1-90 (includes Midnight extrapolation)

    -- Debug notice for extrapolated values
    if level > 80 and EpicTipDB and EpicTipDB.debugMode then
        print(string.format("EpicTip: Using EXTRAPOLATED stat value for level %d (pending Midnight beta finalization)", level))
    end

    local factors = CONVERSION_FACTORS[statType]
    if factors then
        return factors[level] or factors[90] -- Default to max level if not found
    end
    return 1
end
...
function EpicTipStatValuesMixin:ProcessStatTooltip(tooltip)
    ...
    local ok, text = pcall(line1.GetText, line1)
    if not ok or not text or IsSecretValue(text) or type(text) ~= "string" then return end
...
-- Spec/role data: primary stat and role per class+spec.
-- ClassID and SpecID match WoW's internal values (GetSpecializationInfo).
-- Role: "DAMAGER", "TANK", "HEALER"
-- PrimaryStat: "Strength", "Agility", "Intellect"
local SPEC_DATA = {
    -- Death Knight
    [6] = {
        [1] = { role = "TANK",    primary = "Strength"  }, -- Blood
        [2] = { role = "DAMAGER", primary = "Strength"  }, -- Frost
        [3] = { role = "DAMAGER", primary = "Strength"  }, -- Unholy
    },
    -- Demon Hunter
    [12] = {
        [1] = { role = "DAMAGER", primary = "Agility"   }, -- Havoc
        [2] = { role = "TANK",    primary = "Agility"   }, -- Vengeance
        [3] = { role = "DAMAGER", primary = "Intellect" }, -- Aldrachi (Devourer)
    },
    -- Druid
    [11] = {
        [1] = { role = "DAMAGER", primary = "Intellect" }, -- Balance
        [2] = { role = "DAMAGER", primary = "Agility"   }, -- Feral
        [3] = { role = "TANK",    primary = "Agility"   }, -- Guardian
        [4] = { role = "HEALER",  primary = "Intellect" }, -- Restoration
    },
    -- Evoker
    [13] = {
        [1] = { role = "DAMAGER", primary = "Intellect" }, -- Devastation
        [2] = { role = "HEALER",  primary = "Intellect" }, -- Preservation
        [3] = { role = "DAMAGER", primary = "Intellect" }, -- Augmentation
    },
    -- Hunter
    [3] = {
        [1] = { role = "DAMAGER", primary = "Agility"   }, -- Beast Mastery
        [2] = { role = "DAMAGER", primary = "Agility"   }, -- Marksmanship
        [3] = { role = "DAMAGER", primary = "Agility"   }, -- Survival
    },
    -- Mage
    [8] = {
        [1] = { role = "DAMAGER", primary = "Intellect" }, -- Arcane
        [2] = { role = "DAMAGER", primary = "Intellect" }, -- Fire
        [3] = { role = "DAMAGER", primary = "Intellect" }, -- Frost
    },
    -- Monk
    [10] = {
        [1] = { role = "TANK",    primary = "Agility"   }, -- Brewmaster
        [2] = { role = "HEALER",  primary = "Intellect" }, -- Mistweaver
        [3] = { role = "DAMAGER", primary = "Agility"   }, -- Windwalker
    },
    -- Paladin
    [2] = {
        [1] = { role = "HEALER",  primary = "Intellect" }, -- Holy
        [2] = { role = "TANK",    primary = "Strength"  }, -- Protection
        [3] = { role = "DAMAGER", primary = "Strength"  }, -- Retribution
    },
    -- Priest
    [5] = {
        [1] = { role = "HEALER",  primary = "Intellect" }, -- Discipline
        [2] = { role = "HEALER",  primary = "Intellect" }, -- Holy
        [3] = { role = "DAMAGER", primary = "Intellect" }, -- Shadow
    },
    -- Rogue
    [4] = {
        [1] = { role = "DAMAGER", primary = "Agility"   }, -- Assassination
        [2] = { role = "DAMAGER", primary = "Agility"   }, -- Outlaw
        [3] = { role = "DAMAGER", primary = "Agility"   }, -- Subtlety
    },
    -- Shaman
    [7] = {
        [1] = { role = "DAMAGER", primary = "Intellect" }, -- Elemental
        [2] = { role = "DAMAGER", primary = "Agility"   }, -- Enhancement
        [3] = { role = "HEALER",  primary = "Intellect" }, -- Restoration
    },
    -- Warlock
    [9] = {
        [1] = { role = "DAMAGER", primary = "Intellect" }, -- Affliction
        [2] = { role = "DAMAGER", primary = "Intellect" }, -- Demonology
        [3] = { role = "DAMAGER", primary = "Intellect" }, -- Destruction
    },
    -- Warrior
    [1] = {
        [1] = { role = "DAMAGER", primary = "Strength"  }, -- Arms
        [2] = { role = "DAMAGER", primary = "Strength"  }, -- Fury
        [3] = { role = "TANK",    primary = "Strength"  }, -- Protection
    },
}

-- Returns the current player's spec info: { role, primary } or nil
local function GetPlayerSpecInfo()
    ...
    local classSpecs = SPEC_DATA[classID]
    if not classSpecs then return nil end
    -- specID is a global unique ID; find it in this class's specs
    for index, data in pairs(classSpecs) do
        local id = GetSpecializationInfoByID and select(1, GetSpecializationInfoByID(specID))
        if id == specID then return data end
    end
    -- Fallback: use specIndex directly (1/2/3/4)
    return classSpecs[specIndex]
end

function EpicTipStatValuesMixin:GetStatWeights()
    local spec = GetPlayerSpecInfo()

    -- Determine primary stat weight (primary = 1.0, other primaries = 0)
    local strW, agiW, intW = 0, 0, 0
    if spec then
        if spec.primary == "Strength"  then strW = 1.0
        elseif spec.primary == "Agility"   then agiW = 1.0
        elseif spec.primary == "Intellect" then intW = 1.0
        end
    else
        -- No spec info: treat all primaries equally
        strW, agiW, intW = 1.0, 1.0, 1.0
    end

    -- Secondary weights vary by role
    local critW, hasteW, mastW, versW, stamW = 1.0, 1.0, 1.0, 0.8, 0.1
    if spec then
        if spec.role == "TANK" then
            -- Tanks value stamina and versatility more highly
            stamW = 1.0
            versW = 1.2
            critW = 0.7
            hasteW = 0.8
        ...
        elseif spec.role == "HEALER" then
            -- Healers lean on haste and mastery
            hasteW = 1.2
            mastW = 1.1
            critW = 0.9
            versW = 0.9
    ...
    return {
        ITEM_MOD_STRENGTH_SHORT          = strW,
        ITEM_MOD_AGILITY_SHORT           = agiW,
        ITEM_MOD_INTELLECT_SHORT         = intW,
        ITEM_MOD_CRIT_RATING_SHORT       = critW,
        ITEM_MOD_HASTE_RATING_SHORT      = hasteW,
        ITEM_MOD_MASTERY_RATING_SHORT    = mastW,
        ITEM_MOD_VERSATILITY             = versW,
        ITEM_MOD_STAMINA_SHORT           = stamW,
...
function StatValues.DebugStatValues()
    ...
    ET:Print("=== Stat Values Validation ===")
...
-- Bridge: make ET.StatValues callable with mixin methods so Tooltip.lua's ET.StatValues:X() calls work
Mixin(ET.StatValues, EpicTipStatValuesMixin)
