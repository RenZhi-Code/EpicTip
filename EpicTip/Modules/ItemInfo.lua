...
function EpicTipItemInfoMixin:ShowDetailedComparison(tooltip, itemLink, itemEquipLoc)
    ...
    -- Resolve to a single link: single-slot = {link=..., level=...}, multi-slot = {{link=...}, ...}
    local equippedLink = nil
    if equippedItems then
        if equippedItems.link then
            equippedLink = equippedItems.link
        elseif equippedItems[1] and equippedItems[1].link then
...
function EpicTipItemInfoMixin:GetItemSource(itemID)
    ...
    -- Check if it's a quest item — GetItemInfo returns classID as 12th return value
...
function EpicTipItemInfoMixin:ProcessItemTooltip(tooltip)
    ...
    -- Patch 12.0.0+: item link could be secret in certain contexts
    if issecretvalue then
        local ok, isSecret = pcall(issecretvalue, link)
        if ok and isSecret then return end
    end
    ...
    -- GetItemInfo returns multiple values, not a table
    ...
    -- Keystone detection: show dungeon gear/crest rewards via StaticData
    if ET.StaticData and itemSubType and itemSubType:find("Keystone") then
        -- The keystone level is encoded as the item level in the link
        local keyLevel = itemLevel or 0
        if keyLevel > 0 then
            local gearIlvl   = ET.StaticData.GetKeystoneItemLevel(keyLevel, false)
            local vaultIlvl  = ET.StaticData.GetKeystoneItemLevel(keyLevel, true)
            local mpData     = ET.StaticData.GetMythicPlusData()
            local crestData  = mpData and mpData.crestDrops and mpData.crestDrops[math.min(keyLevel, 10)]

            tooltip:AddLine(" ", 1, 1, 1)
            tooltip:AddLine("|cFFFFD700Mythic+ Rewards (Season 1)|r", 1, 1, 1)

            local ilvlData = mpData and mpData.itemLevels and mpData.itemLevels[math.min(keyLevel, 10)]

            if gearIlvl then
                local track = ilvlData and ilvlData.dungeonTrack and (" |cffaaaaaa(" .. ilvlData.dungeonTrack .. ")|r") or ""
                tooltip:AddDoubleLine("End of Dungeon:", tostring(gearIlvl) .. " ilvl" .. track, 0.8, 0.8, 1, 1, 1, 1)
            end
            if vaultIlvl then
                local track = ilvlData and ilvlData.vaultTrack and (" |cffaaaaaa(" .. ilvlData.vaultTrack .. ")|r") or ""
                tooltip:AddDoubleLine("Great Vault:", tostring(vaultIlvl) .. " ilvl" .. track, 0.8, 0.8, 1, 1, 1, 1)
            end
            if crestData then
                tooltip:AddDoubleLine("Crests:", crestData.count .. "x " .. crestData.type, 0.8, 0.8, 1, 1, 0.8, 0)
            end

            -- Track info from dungeon name
            local dungeonName = mpData and mpData.dungeonKeys and name and (function()
                for abbr, dname in pairs(mpData.dungeonKeys) do
                    if name:find(dname, 1, true) then return dname end
                end
            end)()
            if dungeonName then
                local mountName = mpData.dungeonMounts and mpData.dungeonMounts[dungeonName]
                if mountName then
                    tooltip:AddDoubleLine("Mount Drop:", mountName, 0.5, 1, 0.5, 1, 1, 1)
                end
            end
        end
    end
...
-- Show a weighted upgrade % score vs the currently equipped item
function EpicTipItemInfoMixin:ShowUpgradeScore(tooltip, itemLink, itemEquipLoc)
    if not EpicTipStatValuesMixin or not EpicTipStatValuesMixin.CalculateItemValue then return end

    -- Get the equipped item link for this slot
    self:UpdateEquippedItemCache()
    local equippedItems = equippedItemCache.data[itemEquipLoc]
    local equippedLink = nil
    if equippedItems then
    ...
    if not equippedLink then return end -- nothing equipped in slot, skip

    -- Create a temporary mixin to call CalculateItemValue
    local processor = {}
    Mixin(processor, EpicTipStatValuesMixin)

    local newScore    = processor:CalculateItemValue(itemLink)
    local equippedScore = processor:CalculateItemValue(equippedLink)

    if not newScore or not equippedScore or equippedScore <= 0 then return end

    local pct = ((newScore - equippedScore) / equippedScore) * 100

    -- Only show if there's a meaningful difference (>= 0.1%)
    if math.abs(pct) < 0.1 then return end

    -- Get spec name for context
    local specLabel = ""
    local specIndex = GetSpecialization and GetSpecialization()
    if specIndex then
        local _, specName = GetSpecializationInfo(specIndex)
        if specName then
            specLabel = " for " .. specName
        end
    end

    tooltip:AddLine(" ", 1, 1, 1)
    if pct > 0 then
        tooltip:AddLine(string.format("|cff00ff00Upgrade: +%.1f%%%s|r", pct, specLabel), 0, 1, 0)
    else
        tooltip:AddLine(string.format("|cffff4444Downgrade: %.1f%%%s|r", pct, specLabel), 1, 0.27, 0.27)
    end
end
...
-- Bridge: make ET.ItemInfo callable with mixin methods so Tooltip.lua's ET.ItemInfo:X() calls work
Mixin(ET.ItemInfo, EpicTipItemInfoMixin)
