...
-- introduced in Patch 12.0.0 (Midnight). Aura data (C_UnitAuras.GetAuraDataByIndex)
-- may return secret values that tainted code cannot inspect or concatenate.
-- Key protections added:
-- - issecretvalue() checks before accessing auraData.name and auraData.spellId
-- - Graceful skipping of secret auras instead of erroring
-- See: https://warcraft.wiki.gg/wiki/Patch_12.0.0/API_changes
--
-- PATCH 12.0.1 BETA 6 UPDATES:
-- - GetUnitAuraBySpellID/GetPlayerAuraBySpellID are now fixed for non-secret spells
-- - IMPORTANT: Aura Instance IDs are no longer unique across all units!
--   The same ID can now be used on different units for different auras.
--   This module does NOT use aura instance IDs, so no changes needed.
...
-- Safely read a field from a potentially secret table (Patch 12.0.0+)
...
-- C_MountJournal is Retail-only (doesn't exist in Classic/Era)
if WOW_PROJECT_ID ~= WOW_PROJECT_MAINLINE then
    -- Create empty stub for Classic
    MountInfo.GetMountInfo = function() return nil end
    if EpicTipDB and EpicTipDB.debugMode then
        print("EpicTip: MountInfo disabled (Classic/Era detected)")
    end
    return
end
...
function EpicTipMountInfoMixin:IsUnitMounted(unit)
    ...
    if C_UnitAuras and C_UnitAuras.GetAuraDataByIndex then
        local auraIndex = 1
        while auraIndex <= 40 do  -- Safety limit
            local ok, auraData = pcall(C_UnitAuras.GetAuraDataByIndex, unit, auraIndex, "HELPFUL")
            if not ok or not auraData then break end

            -- Patch 12.0.0+: Use SafeField to avoid secret vector propagation
            local name = SafeField(auraData, "name", nil)
            local spellId = SafeField(auraData, "spellId", nil)

            if name and type(name) == "string" then
                if name:find("Mount") or name:find("Flying") or name:find("Riding")
                   or (spellId and self:IsMountAura(spellId)) then
                    return true
                end
            end
...
function EpicTipMountInfoMixin:GetUnitMountID(unit)
    ...
    if C_UnitAuras and C_UnitAuras.GetAuraDataByIndex then
        ...
        while auraIndex <= 40 do  -- Safety limit
            ...
            if name and type(name) == "string" and spellId and self:IsMountAura(spellId) then
                local mountID = self:GetMountIDFromSpell(spellId)
                if mountID then
                    return mountID
                end
            end
...
function EpicTipMountInfoMixin:ProcessUnitTooltip(tooltip, unit)
    ...
    -- Show drop rate if known (StaticData lookup)
    if ET.StaticData and mountInfo.name then
        local dropRate = ET.StaticData.GetMountDropRate(mountInfo.name)
        if dropRate then
            tooltip:AddDoubleLine(L["Drop Rate:"] or "Drop Rate:",
                string.format("%.2f%%", dropRate), 0.7, 1, 0.7, 1, 0.8, 0.2)
        end
    end
