...
local defaults = {
    profile = {
        ...
        scale = 1.0,
        anchoring = "default",
        fixedAnchorPoint = "BOTTOMRIGHT",
        fixedAnchorX = -50,
        fixedAnchorY = 100,
        debugMode = false,
        allowRestrictedCosmetics = false,
        ...
        borderWidth = 2,
        maxTooltipWidth = 550,
        reactionColoredBackground = false,
        classColoredBackground = false,
...
function ET:OnInitialize()
    ...
    -- Migrate/ensure tooltip styling defaults exist (for existing users)
    if not EpicTipDB.backgroundColor then
        EpicTipDB.backgroundColor = { r = 0, g = 0, b = 0, a = 0.8 }
    end
    if not EpicTipDB.borderColor then
        EpicTipDB.borderColor = { r = 0.3, g = 0.3, b = 0.4, a = 1.0 }
    end
    if not EpicTipDB.backgroundOpacity then
        EpicTipDB.backgroundOpacity = 0.8
    end
    if not EpicTipDB.borderWidth then
        EpicTipDB.borderWidth = 2
    end
    if not EpicTipDB.maxTooltipWidth then
        EpicTipDB.maxTooltipWidth = 550
    end
    if EpicTipDB.reactionColoredBackground == nil then
        EpicTipDB.reactionColoredBackground = false
    end
    if EpicTipDB.classColoredBackground == nil then
        EpicTipDB.classColoredBackground = false
    end
    if EpicTipDB.classColoredBorder == nil then
        EpicTipDB.classColoredBorder = false
    end
    if EpicTipDB.backgroundFade == nil then
        EpicTipDB.backgroundFade = false
    end
    if EpicTipDB.allowRestrictedCosmetics == nil then
        EpicTipDB.allowRestrictedCosmetics = false
    end
...
function ET:OnEnable()
    ...
    -- INSPECT_HONOR_UPDATE fires when RequestInspectHonorData() returns data
    -- Used by PvPInfo to show arena/RBG ratings for inspected players
    if RequestInspectHonorData then
        self:RegisterEvent("INSPECT_HONOR_UPDATE")
    end
end

-- Event handlers are implemented in Core/Events.lua.
-- Keep Core.lua free of event handler definitions to avoid overriding the taint-safe versions.
...
function ET:SlashCommand(input)
    ...
    elseif input == "anchor" or input == "anchoring" then
        ...
        local cycle = { default = "mouse", mouse = "smart", smart = "fixed", fixed = "default" }
        local labels = { default = "Default Position", mouse = "Follow Mouse", smart = "Smart Positioning", fixed = "Fixed Position" }
        local new = cycle[current] or "default"

        self.db.profile.anchoring = new
        EpicTipDB.anchoring = new

        print(string.format("%s Tooltip positioning: %s", ET.CHAT_PREFIX, labels[new] or new))
