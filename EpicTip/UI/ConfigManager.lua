...
function ConfigManager.GetOptionsTable()
    ...
    if not ET.UI or not ET.UI.Config then
        return {
            name = "|cFFFF0000Epic|r|cFF33CCFFTip|r v" .. (ET.VERSION or "27.03.26.5g"),
            handler = ET,
            type = "group",
            args = {
                loading = {
...
function ConfigManager.InitializeBlizzardOptions()
    ...
    -- THEN add to Blizzard's Interface Options (only if not already added)
    if not AceConfigDialog.BlizOptions["EpicTip"] then
