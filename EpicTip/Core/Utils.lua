...
function EpicTipUtilsMixin:OnLoad()
    ...
    -- C_Timer is available in Retail and most Classic versions, but check for safety
...
function EpicTipUtilsMixin:GetSpec(unit)
    ...
    -- Fallback for self or when inspect data not yet available
    if (not specID or specID == 0) and UnitIsUnit(unit, "player") then
        local idx = GetSpecialization and GetSpecialization()
        if idx then
            specID = GetSpecializationInfo and GetSpecializationInfo(idx)
        end
    end
...
function EpicTipUtilsMixin:CanInspectThrottled(unit)
    ...
    -- Check if unit is in inspection range BEFORE calling CanInspect
    -- This prevents "too far away" errors in Classic/Era
